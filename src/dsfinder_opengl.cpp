...
OpenGLGetInfo(b32 ModernContext)
{
    ...
    while (*At)
    {
        ...
        // TODO(vincent): Normally you'd test for extensions you want to fetch here and set
        // Result.stuff = true;
...
{
    // NOTE(vincent): The game sends vectoors to the OpenGL renderer in pixel space
    // (they were converted from meter space or internal game world space 
    // when we pushed the rendering commands). But OpenGL expects us to feed it vertices 
    // that are in unit cube space, where coordinates go from -1 to 1 for both the x and y axis.
    // In OpenGL there is this concept of MVP matrix (product of model, view and projection matrices)
    // which is a 4*4 matrix that transforms an incoming (x,y,z,w) vector.
    // We can specify each matrix individually but we will only set up the projection matrix
    // to do the actual transformation. The w coordinate is used by the hardware to do a non-linear
    // transformation afterwards (dividing every other component, and maybe other things), 
    // which has to do with 3D perspective stuff I'm not very familiar with. 
    // Since we are a simple 2D game we are only really concerned with transforming 
    // the x and y coordinates. The z and w will be preserved, 
    // so the last two rows of our matrix will be like the identity matrix.
    ...
    f32 ProjectionMatrix[] =    // transpose this in your head to get the actual math layout
    {
        a,   0, 0, 0,
        0,   b, 0, 0,
        0,   0, 1, 0,
...
OpenGLRenderToOutput(game_render_commands *Commands, s32 WindowWidth, s32 WindowHeight)
{
    ...
    // NOTE(vincent): The pipeline stages of GPUs after the fragment shader 
    // are typically not very programmable. So you won't be able to program shaders for the blending.
    // Instead, you are provided glBlendFunc(), where you can pass a few enumerated values for (a,b)
    // hwere the final result pixel is calculated as such: NewDest = a*Dest + b*Source.
    ...
    for (u32 Address = 0; Address < Commands->PushBufferSize;)
    {
        ...
        switch (Header->Type)
        {
            case RenderEntryType_render_entry_clear:
            {
                ...
                glClearColor(Entry->Color.r, Entry->Color.g, Entry->Color.b,
                             Entry->Color.a);
            ...
            case RenderEntryType_render_entry_bitmap:
            {
                ...
                else
                {
                    ...
                    glTexImage2D(GL_TEXTURE_2D, 0, GL_RGBA8, 
                                 Entry->Bitmap->Width, Entry->Bitmap->Height, 
            ...
            case RenderEntryType_render_entry_bitmap_nearest:
            {
                render_entry_bitmap *Entry = (render_entry_bitmap *)Data;
                Assert(Entry->Bitmap);
