...
PLATFORM_GET_INITIAL_PATH(LinuxGetInitialPath)
{
    Assert(MaxSize >= 2);
    char *Location = getcwd(Path->Base, MaxSize+1);
    if (Location == Path->Base)
    {
        while (*Location)
            ++Location;
        Path->Size = Location - Path->Base;
    }
    else
    {
        Path->Base[0] = '/';
...
PLATFORM_APPEND_DIRNAME(LinuxAppendDirname)
{
    b32 Appended = false;
...
PLATFORM_POP_DIRNAME(LinuxPopDirname)
{
    b32 Popped = false;
    Assert(Path->Size >= 1);
    Assert(Path->Base[0] == '/');
...
PLATFORM_PUSH_FILENAMES(LinuxPushFilenames)
{
    filenames Filenames = {};
    DIR *Directory = opendir(Dirname);
    if (!Directory)
    {
        //fprintf(stderr, "opendir failed. DirPath: %s\n", Dirname);
    }
    else
    {
        for (;;)
        {
            ...
            if (!Entry)
                break;
            if (StringEndsWithCI(Entry->d_name, ".sm") ||
                (Entry->d_type == DT_DIR && NeitherDotNorDotDot(Entry->d_name)))
            {
                ++Filenames.Count;
            }
        }
        ...
        //if (!Directory)
        //fprintf(stderr, "second opendir failed\n");
        //else
        if (Directory)
        {
            while (FileIndex < Filenames.Count)
            {
                struct dirent *Entry = readdir(Directory);
                if (!Entry)
                    break;
                if (StringEndsWithCI(Entry->d_name, ".sm") ||
                    (Entry->d_type == DT_DIR && NeitherDotNorDotDot(Entry->d_name)))
                {
                    Filenames.IsDirectory[FileIndex] = Entry->d_type == DT_DIR;
                    Filenames.Strings[FileIndex] = PushString(Arena, Entry->d_name);
                    ++FileIndex;
                }
            }
            Assert(FileIndex == Filenames.Count);
            closedir(Directory);
        }
    }
    return Filenames;
...
{
#define SO_FILENAME "dsfinder.so"
#define SO_COPY_FILENAME "dsfinder_copy.so"
    void *DLL;
    game_update *Update;
...
#else
    Code->DLL = dlopen("./" SO_FILENAME, RTLD_LAZY);
#endif
    if (!Code->DLL)
...
global_variable f32 NormalizedRX;
global_variable f32 NormalizedRY;
internal void
...
#define SetPressed(Code, Button)  case Code: Input->Button.IsPressed = IsDown; break
            ...
            case XCB_BUTTON_RELEASE:
            {
                ...
                switch (BP->detail)
                {
                    SetPressed(1, MouseLeft);       // mouse left
                }
            } break;
    ...
    // - The problem at hand is We want to let the controller disconnect and reconnect
    ...
    if (C->JoystickFD >= 0)
    {
        ...
        while (read(C->JoystickFD, &JoystickEvent, sizeof(js_event)) == sizeof(js_event))
        {
            ...
            switch (JoystickEvent.type)
            {
                ...
                case JS_EVENT_AXIS:
                {
                    ...
                    // left stick Y: number 1, value -32767 to 32767 top to bottom (need to invert)
                    ...
                    switch (JoystickEvent.number)
                    {
                        ...
                        case 3:
                        {
                            NormalizedRX = JoystickEvent.value / 32767.0f;
                            RStickMoved = true;
                        } break;
                        case 4:
                        {
                            NormalizedRY = -JoystickEvent.value / 32767.0f;
                            RStickMoved = true;
                        } break;
                    }
        ...
        if (RStickMoved)
        {
            f32 Magnitude = SquareRoot(Square(NormalizedRX) + Square(NormalizedRY));
            f32 Deadzone = .25f;
            f32 ProcessedMagnitude = 0.0f;
            if (Magnitude > Deadzone)
                ProcessedMagnitude = (Magnitude - Deadzone) / (1.0f - Deadzone);
            f32 MagnitudeRatio = SafeRatio0(ProcessedMagnitude, Magnitude);
            Input->ThumbRX = MagnitudeRatio * NormalizedRX;
            Input->ThumbRY = MagnitudeRatio * NormalizedRY;
        }
    }
}
...
int main(void)
{
    ...
    GameMemory.Platform.PushFilenames = LinuxPushFilenames;
    GameMemory.Platform.GetInitialPath = LinuxGetInitialPath;
    GameMemory.Platform.AppendDirname = LinuxAppendDirname;
    GameMemory.Platform.PopDirname = LinuxPopDirname;
